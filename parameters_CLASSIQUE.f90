 ...
 ! --- Misc ---
  ...
  ! --- Paraterers ---
   ...
   parameter ( f0 = 1.e-4, beta = 0) ! 1.e-11 )
 
   parameter ( r_drag = 1.e-7 )
 
   parameter ( r_invLap = 0 ) !1.e-6*twopi**2/Ly**2 )
  ...
  ! ---  Time ---
   ...
   parameter ( ndays= 365, totaltime = 86400 * ndays ) !59
 
   parameter ( nsteps = totaltime/dt+1 ,fileperday= 12)
 ...
 ! parameter ( iout = 9 , i_diags = ifix(86400./16/dt) )
   ...
   parameter ( start_movie = 1. , start_spec=1., subsmprto=1, ftsubsmprto=1, save_movie=.true. )
 ...
 !  parameter ( restart = .true. , use_ramp = .false. )
   parameter ( restart = .true. , use_ramp = .false., gaussian_bump_eta = .false.)
 ...
 ! --- Slab model/coupling switches --- 
   ...
   parameter ( slab_layer = .false.) !!! Do we want a slab layer? 

   parameter ( cou=.false., wind_top = .true., wind_slab = .false.) !!! Coupling vs Wind on top layervs wind on slab layer (Out of these three, only one can be .true. here)
   
   parameter ( ustar=.false., waves=.false., stokes=.false.) !!! Coupling activation.
   
   parameter ( tau_max = 0.1, step=0.0 )

...
for step in step_array : 
    ...
    nti = ntf - 10
    dx  = 2000000/nx
    Keys = ['zeta_ek','div_ek','zeta1','div1','u_o','v_o']
    ds = rd.readat(Keys,
                   path = filepath,
                   nt=ntf, nti=nti,
                   nx=nx, ny = ny)
    ...
    # --- Creating the figure
    Plots = [ds.zeta_ek, ds.zeta_ek, ds.zeta1, zeta2,
             ds.div_ek, ds.div_ek, ds.div1, div2]
    ...
    plt.tight_layout()
    plt.savefig('8pannels_DivRot_CHEN_step{}.png'.format(step))
    plt.show()
    plt.close()

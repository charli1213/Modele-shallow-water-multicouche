...
brancheA_path = basepath+'RECOU{}_np38_tau0.10_step1.0/data/'
brancheA_nt   = [10]

# --- BRANCHE B params (Non couplé)
brancheB_path = basepath + 'RESTARTCHEN_5y_tau0.09_512_step0.01/data/'
...
# --- FINAL ::
total = vent + nabBc + nabB + dE_SC + dE_CL + dE_B + dE_A
...
ax5 = plt.subplot2grid(gridsize, (1, 0), colspan=1, rowspan=1, aspect=1)
ax6 = plt.subplot2grid(gridsize, (1, 1), colspan=1, rowspan=1, aspect=1)
ax7 = plt.subplot2grid(gridsize, (1, 2), colspan=1, rowspan=1, aspect=1)
...
dE_A.isel( time=-1).plot(ax=ax1,vmax=VMax,vmin=-VMax,cmap='RdBu_r')
BdE_A.isel(time=-1).plot(ax=ax5,vmax=VMax,vmin=-VMax,cmap='RdBu_r')

# --- Terme B
VMax = 0.6*max(abs(dE_B.isel( time=-1)).max(),abs(BdE_B.isel( time=-1)).max())
dE_B.isel( time=-1).plot(ax=ax2,vmax=VMax,vmin=-VMax,cmap='RdBu_r')
BdE_B.isel(time=-1).plot(ax=ax6,vmax=VMax,vmin=-VMax,cmap='RdBu_r')

# --- Terme associé à la fontion de Bernouilli
VMax = 0.7*max(abs(BnabB.isel( time=-1)).max(),abs(nabB.isel( time=-1)).max())
nabB.isel( time=-1).plot(ax=ax3,vmax=VMax,vmin=-VMax,cmap='RdBu_r')
BnabB.isel(time=-1).plot(ax=ax7,vmax=VMax,vmin=-VMax,cmap='RdBu_r')

# --- Terme associé au vent Vent
VMax = 0.8*max(abs(vent.isel( time=-1)).max(),abs(Bvent.isel( time=-1)).max())
vent.isel( time=-1).plot(ax=ax4,vmax=VMax,vmin=-VMax,cmap='RdBu_r')
Bvent.isel(time=-1).plot(ax=ax8,vmax=VMax,vmin=-VMax,cmap='RdBu_r')


#dE_CL.isel(time=-1).plot(ax=ax0,label='Craik-Leibovich',color='seagreen')
#dE_SC.isel(time=-1).plot(ax=ax1,label="Stokes-Coriolis",color='seagreen')
...
# --- Fig params
ax1.set_title(r"zeta_ek x u1 (c) ")

ax2.set_title(r"(zeta_1 + zeta_ek) x Uek (c)")

ax3.set_title(r"Bernouilli (c)")

ax4.set_title(r"$\tau_{ocean}$ (c)")


ax5.set_title(r"zeta_ek x u1 (nc) ")

ax6.set_title(r"(zeta_1 + zeta_ek) x Uek (nc)")

ax7.set_title(r"Bernouilli (nc)")

ax8.set_title(r"$\tau_{ocean}$ (nc)")

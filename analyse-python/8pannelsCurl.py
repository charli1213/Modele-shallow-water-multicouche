...
# 8 snapshot du curl/div tous les termes RHS communs.
...
taux = -0.1*np.cos(2*np.pi*Y/nx)
Bvent = rd.div(taux*(Uek/Uek),taux*(Uek/Uek)*0)/(1000*hek**2)
...
# --- Terme A
VMax = 0.4*max(abs(dA_A.isel( time=-1)).max(),abs(BdA_A.isel( time=-1)).max())
dA_A.isel( time=-1).plot(ax=ax1,vmax=VMax,vmin=-VMax,cmap='RdBu_r')
BdA_A.isel(time=-1).plot(ax=ax5,vmax=VMax,vmin=-VMax,cmap='RdBu_r')

# --- Terme B
VMax = 0.2*max(abs(dA_B.isel( time=-1)).max(),abs(BdA_B.isel( time=-1)).max())
dA_B.isel( time=-1).plot(ax=ax2,vmax=VMax,vmin=-VMax,cmap='RdBu_r')
BdA_B.isel(time=-1).plot(ax=ax6,vmax=VMax,vmin=-VMax,cmap='RdBu_r')

# --- Terme associé à la fontion de Bernouilli
VMax = 0.4*max(abs(BnabB.isel( time=-1)).max(),abs(nabB.isel( time=-1)).max())
...
VMax = 0.7*max(abs(vent.isel( time=-1)).max(),abs(Bvent.isel(time=-1)).max())

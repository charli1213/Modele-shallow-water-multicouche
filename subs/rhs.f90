...
!
       ...
       ! ### --- Modification CEL --- !
       ! Internal forcing on top layer : parameter(wind_top=.true.)
       if ((k .eq. 1) .and. (wind_top)) then
          rhs_u(i,j,k) = rhs_u(i,j,k) &
               &       + (1+step*SIN(it*f0*dt))*tau_max*SIN(twopi*jm/ny*1.)/(1000*0.5*(thickness(i,j)+thickness(im,j)))
       end if
       ! ### --- Modification CEL --- !
       ...
       rhs_eta(i,j,k) = -(uh(ip,j)-uh(i,j))/dx   &
          &             -(vh(i,jp)-vh(i,j))/dy   & 
          &             -top(k)*( &
          &              (Uek(ip,j,2)-Uek(i,j,2))/dx & 
          &             +(Vek(i,jp,2)-Vek(i,j,2))/dy )
       enddo
       enddo

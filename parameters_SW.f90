 ...
 ! --- Misc ---
  ...
  ! ---  Time ---
   ...
   !parameter ( ndays= 36500, totaltime = 86400 * ndays )
   parameter ( ndays= 36500, totaltime = 86400 * ndays )
 
   parameter ( nsteps = totaltime/dt +1 ,fileperday=0.03125) !4*24) ! Output aux 32 jours. 
 ...
 !  parameter ( restart = .true. , use_ramp = .false. )
   parameter ( restart = .false. , use_ramp = .false., gaussian_bump_eta = .false.)
 ...
 ! --- Slab model/coupling switches --- 
   ...
   parameter ( slab = .false.) !!! Slab layer activation.

   parameter ( cou = .false., ustar=.false., waves=.false.) !!! Coupling activation.

   parameter ( wind = .true.) !!! Wind on top layer.

   parameter ( tau_max = 0.1 )
